...
// Check the tangential continuity (H(curl) conformity) of FE_FaceNedelec in 3d
// when wrapped in an FESystem, across an interior face for all non-standard
// orientation configurations. This exercises the composition of the
// face-orientation permutation and sign-change tables through FESystem, which
// is the way the element is typically used in applications.
  ...
  try
    {
      ...
      for (unsigned int fe_degree = 0; fe_degree < 3; ++fe_degree)
        {
          // H(curl) conformal
          FESystem<dim> fe(FE_FaceNedelec<dim>(fe_degree), 1);
